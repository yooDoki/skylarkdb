...
fn filter_routine_params(rtype: &str, mut params: Vec<MySQLRoutineParam>) -> Vec<MySQLRoutineParam> {
    if rtype == "FUNCTION" {
        params.retain(|p| !(p.ordinal == 0 && p.name.is_none()));
    }
    params
}

pub async fn list_routines(connection_id: &str) -> Result<Vec<MySQLRoutine>, String> {
    let pool = {
        let connections = MYSQL_CONNECTIONS.lock().await;
        connections.get(connection_id).cloned().ok_or("Connection not found")?
    };
    ...
    let routine_rows: Vec<(String, String, String, Option<String>, Option<String>)> =
    ...
    let param_rows: Vec<(String, String, i32, Option<String>, Option<String>, String)> =
        if let Some(ref db) = default_db {
            sqlx::query_as(
                "SELECT specific_schema, specific_name, ordinal_position, parameter_name, parameter_mode, data_type
        ...
        .map_err(|e| format!("Failed to list routine parameters: {}", e))?;

    let mut param_map: HashMap<(String, String), Vec<MySQLRoutineParam>> = HashMap::new();
    for (schema, name, ord, pname, pmode, dtype) in param_rows {
        param_map
            .entry((schema, name))
            .or_default()
            .push(MySQLRoutineParam {
                ordinal: ord,
                name: pname,
                mode: pmode,
                data_type: dtype,
    ...
    let mut routines: Vec<MySQLRoutine> = Vec::new();
    for (schema, name, rtype, data_type, def) in routine_rows {
        let key = (schema.clone(), name.clone());
        let raw_params = param_map.remove(&key).unwrap_or_default();
        let params = filter_routine_params(&rtype, raw_params);
        let def_preview = def.map(|d| {
            if d.len() > 4000 {
                format!("{}…", &d[..4000])
            } else {
                d
