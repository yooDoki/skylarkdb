...
pub async fn export_table_to_sql(
    ...
    if include_structure {
        ...
        writer
            .write_all(b"-- Table structure for `")
            .map_err(|e| format!("Failed to write to file: {}", e))?;
        writer
            .write_all(table_name.as_bytes())
            .map_err(|e| format!("Failed to write to file: {}", e))?;
        writer
            .write_all(b"`\n")
            .map_err(|e| format!("Failed to write to file: {}", e))?;
        writer
            .write_all(b"DROP TABLE IF EXISTS `")
            .map_err(|e| format!("Failed to write to file: {}", e))?;
        writer
            .write_all(table_name.as_bytes())
            .map_err(|e| format!("Failed to write to file: {}", e))?;
...
fn extract_value_as_json_string(row: &sqlx::mysql::MySqlRow, index: usize) -> String {
    ...
    if let Ok(v) = row.try_get::<Option<f64>, _>(index) {
        return match v {
            Some(n) => {
                serde_json::Number::from_f64(n).map_or("null".to_string(), |n| n.to_string())
            }
            None => "null".to_string(),
        };
    }
...
fn extract_value_as_sql_string(row: &sqlx::mysql::MySqlRow, index: usize) -> String {
    ...
    if let Ok(v) = row.try_get::<Option<bool>, _>(index) {
        return v.map_or("NULL".to_string(), |b| {
    ...
    if let Ok(v) = row.try_get::<Option<chrono::NaiveDateTime>, _>(index) {
        return v.map_or("NULL".to_string(), |dt| {
            format!("'{}'", dt.format("%Y-%m-%d %H:%M:%S"))
        });
    }

    if let Ok(v) = row.try_get::<Option<chrono::NaiveDate>, _>(index) {
        return v.map_or("NULL".to_string(), |d| {
...
pub async fn export_database(
    ...
    for table in &options.tables {
        let table_output_path = match options.format {
            ExportFormat::Json => Path::new(&options.output_path).join(format!("{}.json", table)),
            ExportFormat::Csv => Path::new(&options.output_path).join(format!("{}.csv", table)),
            ExportFormat::Sql => Path::new(&options.output_path).join(format!("{}.sql", table)),
        };
    ...
    Ok(ExportResult {
        success: true,
        message: format!(
            "Successfully exported {} tables with {} total rows",
            exported_tables, total_rows
        ),
